      ...
      <v-flex xs12 md4>
        <v-card>
          <v-card-text>
            ...
            <v-alert :value="true" type="warning" v-if="error_login">
              Usuario o Password equivocado.
            </v-alert>
            <v-form @submit.prevent="login">
              <v-text-field v-model="form.email" label="Ingrese su correo electronico" required :error-messages="emailErrors" @input="$v.form.email.$touch()" @blur="$v.form.email.$touch()" ></v-text-field>
              <v-text-field v-model="form.password" :append-icon="showPassword ? 'visibility' : 'visibility_off'" :type="showPassword ? 'text' : 'password'" label="Ingrese su password" counter @click:append="showPassword = !showPassword" required :error-messages="passwordErrors" @input="$v.form.password.$touch()" @blur="$v.form.password.$touch()" ></v-text-field>
